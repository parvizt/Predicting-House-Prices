...
# Converting 'TRUE' and 'FALSE' values to 1 and 0
...
def predict_price():
    # Extracting features from user inputs
    ...
    # Creating a new DataFrame based on user inputs
    ...
    # Using a DecisionTreeRegressor model
    ...
    result_label.config(text=f"Predicted Price: {predicted_price[0]:,.0f} Tomans")

    # Calculating the error
    y_pred = model.predict(X_test)
    mae = mean_absolute_error(y_test, y_pred)
    error_percentage = (mae / df['Price'].mean()) * 100
    error_label.config(text=f"Average Error: {error_percentage:.2f}%")

# Creating the user interface
root = tk.Tk()
root.title("DecisionTreeRegressor Model: House Price Estimation")

# Creating widgets and labels
...
error_label = ttk.Label(root, text="Average Error: -")

# Displaying widgets in a grid
